...
def error_rate(predictions, labels):
    ...
    return 100.0 - (
        100.0 *
        numpy.sum(numpy.argmax(predictions, axis=1) == numpy.argmax(labels, axis=1)) /
        predictions.shape[0])

def calc_precision(predictions, labels): # not correct
    """Return the error rate based on dense predictions and 1-hot labels."""
    return (
        100.0 *
        numpy.sum((numpy.argmax(predictions, axis=1) == numpy.argmax(labels, axis=1)) & (numpy.argmax(predictions, axis=1) == 1)) /
        np.count_nonzero(predictions == 1))

def calc_recall(predictions, labels): # not correct
    """Return the error rate based on dense predictions and 1-hot labels."""
    return (
        100.0 *
        numpy.sum((numpy.argmax(predictions, axis=1) == numpy.argmax(labels, axis=1)) & (numpy.argmax(predictions, axis=1) == 0)) /
        np.count_nonzero(predictions == 0))


def our_f1_score(predictions, labels):
    precision = calc_precision(predictions, labels)
    recall = calc_recall(predictions, labels)
            ...
            for iepoch in range(num_epochs):
                ...
                for step in range (int(train_size / BATCH_SIZE)):
                    ...
                    if step % RECORDING_STEP == 0:
                        ...
                        print ('F1 Score: %.3f' % (f1_score(np.argmax(batch_labels, axis=1),np.argmax(predictions, axis=1))))
                        # using scikit-learn, but need to aggregate the labels and predictions since they are matrices
